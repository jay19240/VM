...
/** Gestionnaire singleton chargé de créer et de générer les niveaux mipmap des textures 2D. */
export class Gfx3MipmapManager {
  ...
  /** Initialise l'échantillonneur, le module WGSL et le cache de pipelines par format. */
  constructor() {
  ...
  /**
   * Crée une texture GPU depuis une image ou un canevas et génère tous ses niveaux mipmap.
   *
   * @param bitmap - Image source.
   * @param is8bit - Utilise le format monochrome 8 bits lorsque cette valeur vaut `true`.
   * @param samplerDescriptor - Configuration personnalisée de l'échantillonneur WebGPU.
  ...
  /**
   * Génère les niveaux mipmap d'une texture à partir de son niveau zéro.
   *
   * @param texture - Texture 2D cible, créée avec l'usage `RENDER_ATTACHMENT`.
   * @throws Une erreur si la texture n'est pas bidimensionnelle.
   */
...
/** Instance partagée du gestionnaire de mipmaps. */
