...
/**
 * Clavier virtuel navigable au moyen d'actions directionnelles.
 * Émet `E_VALUE_CHANGED` avec les données `{ value }` lorsque la touche de validation est activée.
 */
export class UIInputKeyboard extends UIWidget {
  ...
  /** Crée un clavier virtuel vide, positionné sur sa première touche. */
  constructor() {
    super({
  ...
  /** Donne le focus au clavier et à la touche correspondant à la position courante. */
  focus(): void {
  ...
  /** Retire le focus du clavier et de la touche courante. */
  unfocus(): void {
    const items = this.node.querySelectorAll('.js-item');
    const item = items[this.row * GRID_WIDTH + this.column];
  ...
  /**
   * Définit et affiche la valeur saisie.
   *
   ...
   *
   * @returns La valeur courante du clavier.
   */
  getValue(): string {
    return this.value;
  }
  ...
  /**
   * Déplace le curseur ou active la touche courante selon l'action reçue.
   * Émet `E_VALUE_CHANGED` lors de l'activation de la touche `ENTER`.
   *
