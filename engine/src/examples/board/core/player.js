import { AIMinMaxStateLeaf, AIMinMaxStateNode } from '@lib/ai/ai_minmax_state';
...
class AIPlayer extends PlayerAbstract {
  ...
  play(game) {
    ...
    let tree = new AIMinMaxStateNode();
  ...
  generateMinMaxTree(node, board, depth, color) {
    if (depth == 0) {
      let leaf = new AIMinMaxStateLeaf(this.evaluate(board));
    ...
    for (let coord of board.findMovables(color)) {
      for (let path of board.getPaths(coord)) {
        ...
        let child = new AIMinMaxStateNode();
        child.setData({ path });
        node.addChild(child);
